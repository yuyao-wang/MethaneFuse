#!/usr/bin/env bash
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
cd "$SCRIPT_DIR"

if [ -f "$HOME/miniconda3/etc/profile.d/conda.sh" ]; then
  # shellcheck disable=SC1091
  source "$HOME/miniconda3/etc/profile.d/conda.sh"
  conda activate panopticon
fi

export PYTHONUNBUFFERED=1
export PYTHONWARNINGS="${PYTHONWARNINGS:-ignore}"

TMP_ROOT="${TMP_ROOT:-/transferdiniu2/yuyao/temp}"
mkdir -p "$TMP_ROOT"
chmod 700 "$TMP_ROOT"
export TMPDIR="$TMP_ROOT"
export TMP="$TMP_ROOT"
export TEMP="$TMP_ROOT"

DATA_ROOT=/mnt/engg-leung/Research_No9_Methane_Emissions/Yuyao/finalDataset_query/legacy_param_480m
TRAIN_CSV="$DATA_ROOT/manifest_time_train.csv"
TEST_CSV="$DATA_ROOT/manifest_time_test.csv"
WEIGHTS="${WEIGHTS:-weights/panopticon_vitb14_teacher.pth}"
CACHE_DIR="${CACHE_DIR:-/diniuvol/yuyao/local_train_temp_cache_480m}"

STAGE_A_CHECKPOINT_DIR="${STAGE_A_CHECKPOINT_DIR:-/transferdiniu2/yuyao/checkpoints/multi_sensor_shared_stage_a}"
STAGE_A_RUN_NAME="${STAGE_A_RUN_NAME:-shared_stageA_baseline_480m_e4_max}"
STAGE_A_CKPT="${STAGE_A_CKPT:-$STAGE_A_CHECKPOINT_DIR/$STAGE_A_RUN_NAME/ckpt_best_test.pth}"
LOG_DIR="${LOG_DIR:-$STAGE_A_CHECKPOINT_DIR/logs}"
mkdir -p "$LOG_DIR"

MOE_CHECKPOINT_DIR="${MOE_CHECKPOINT_DIR:-/transferdiniu2/yuyao/checkpoints/multi_sensor_moe_adapter}"
MOE_RUN_NAME="${MOE_RUN_NAME:-moe_postblock_stageB_from_shared_stageA}"

LORA_CHECKPOINT_DIR="${LORA_CHECKPOINT_DIR:-/transferdiniu2/yuyao/checkpoints/multi_sensor_qv_lora_adapter}"
LORA_RUN_NAME="${LORA_RUN_NAME:-qv_lora_stageB_from_shared_stageA}"

LORAMOE_CHECKPOINT_DIR="${LORAMOE_CHECKPOINT_DIR:-/transferdiniu2/yuyao/checkpoints/multi_sensor_qv_loramoe_adapter}"
LORAMOE_STAGE_B_RUN_NAME="${LORAMOE_STAGE_B_RUN_NAME:-qv_loramoe_stageB_from_shared_stageA}"

STAGE_A_GPU="${STAGE_A_GPU:-1}"
MOE_GPU="${MOE_GPU:-1}"
LORA_GPU="${LORA_GPU:-0}"
LORAMOE_GPU="${LORAMOE_GPU:-0}"

[ -f "$TRAIN_CSV" ] || { echo "Missing TRAIN_CSV: $TRAIN_CSV" >&2; exit 1; }
[ -f "$TEST_CSV" ] || { echo "Missing TEST_CSV: $TEST_CSV" >&2; exit 1; }
[ -f "$WEIGHTS" ] || { echo "Missing WEIGHTS: $WEIGHTS" >&2; exit 1; }

run_logged() {
  local name="$1"
  local gpu="$2"
  shift 2
  echo "[RUN][$name] GPU=$gpu"
  (
    export CUDA_VISIBLE_DEVICES="$gpu"
    "$@" 2>&1 | tee "$LOG_DIR/${name}.log"
  )
}

wait_jobs() {
  local rc=0
  local item pid name
  for item in "$@"; do
    pid="${item%%:*}"
    name="${item#*:}"
    if wait "$pid"; then
      echo "[OK] $name finished"
    else
      echo "[FAIL] $name failed (pid=$pid)" >&2
      rc=1
    fi
  done
  return "$rc"
}

if [ -f "$STAGE_A_CKPT" ]; then
  echo "[SKIP][stage_a] Reusing existing checkpoint: $STAGE_A_CKPT"
else
  run_logged stage_a_baseline "$STAGE_A_GPU" \
    python universal_models_fusion/multi_sensor_panopticon_4.py \
      --train_csv "$TRAIN_CSV" \
      --test_csv "$TEST_CSV" \
      --weights "$WEIGHTS" \
      --device cuda \
      --checkpoint_dir "$STAGE_A_CHECKPOINT_DIR" \
      --local_cache_dir "$CACHE_DIR" \
      --local_cache_min_free_gb 200 \
      --batch_size 32 \
      --epochs 4 \
      --num_workers 8 \
      --train_backbone \
      --head_lr 1e-3 \
      --backbone_lr 5e-5 \
      --row_fusion_mode max \
      --sensor_aux_loss_weight 0.3 \
      --use_wandb \
      --wandb_project query_dataset \
      --wandb_run_name "$STAGE_A_RUN_NAME"
fi

[ -f "$STAGE_A_CKPT" ] || { echo "Missing STAGE_A_CKPT after Stage A: $STAGE_A_CKPT" >&2; exit 1; }
echo "[STAGE_A] Using checkpoint: $STAGE_A_CKPT"

run_logged moe_postblock "$MOE_GPU" \
  python universal_models_fusion/multi_sensor_panopticon_4_nonlinear_moeadapter.py \
    --stage_a_checkpoint "$STAGE_A_CKPT" \
    --train_csv "$TRAIN_CSV" \
    --test_csv "$TEST_CSV" \
    --weights "$WEIGHTS" \
    --device cuda \
    --checkpoint_dir "$MOE_CHECKPOINT_DIR" \
    --local_cache_dir "$CACHE_DIR" \
    --local_cache_min_free_gb 200 \
    --batch_size 32 \
    --epochs 50 \
    --num_workers 8 \
    --head_lr 1e-3 \
    --backbone_lr 5e-5 \
    --adapter_lr 1e-4 \
    --training_schedule vit_then_moe \
    --stage_a_epochs 0 \
    --stage_b_adapter_lr 1e-4 \
    --adapter_bottleneck_dim 16 \
    --adapter_alpha 16 \
    --adapter_dropout 0.05 \
    --adapter_token_blocks 6 \
    --row_fusion_mode max \
    --sensor_aux_loss_weight 0.3 \
    --disable_summary_head \
    --sensor_stats_cache "$MOE_CHECKPOINT_DIR/sensor_stats_moe_adapter.json" \
    --sensor_stats_seed 42 \
    --use_wandb \
    --wandb_project query_dataset \
    --wandb_run_name "$MOE_RUN_NAME" &
PID_MOE=$!

run_logged qv_lora "$LORA_GPU" \
  python universal_models_fusion/multi_sensor_panopticon_4_lora_adapter.py \
    --stage_a_checkpoint "$STAGE_A_CKPT" \
    --train_csv "$TRAIN_CSV" \
    --test_csv "$TEST_CSV" \
    --weights "$WEIGHTS" \
    --device cuda \
    --checkpoint_dir "$LORA_CHECKPOINT_DIR" \
    --local_cache_dir "$CACHE_DIR" \
    --local_cache_min_free_gb 200 \
    --batch_size 24 \
    --epochs 50 \
    --num_workers 8 \
    --head_lr 1e-3 \
    --backbone_lr 5e-5 \
    --adapter_lr 1e-4 \
    --stage_a_epochs 0 \
    --adapter_blocks 12 \
    --adapter_bottleneck_dim 16 \
    --adapter_alpha 16 \
    --adapter_dropout 0.05 \
    --train_backbone \
    --sensor_aux_loss_weight 0.4 \
    --use_wandb \
    --wandb_project query_dataset \
    --wandb_run_name "$LORA_RUN_NAME" &
PID_LORA=$!

run_logged qv_loramoe "$LORAMOE_GPU" \
  python universal_models_fusion/multi_sensor_panopticon_4_loramoe_adapter.py \
    --stage b \
    --stage_a_checkpoint "$STAGE_A_CKPT" \
    --train_csv "$TRAIN_CSV" \
    --test_csv "$TEST_CSV" \
    --weights "$WEIGHTS" \
    --device cuda \
    --checkpoint_dir "$LORAMOE_CHECKPOINT_DIR" \
    --local_cache_dir "$CACHE_DIR" \
    --local_cache_min_free_gb 200 \
    --batch_size 24 \
    --epochs 50 \
    --num_workers 8 \
    --head_lr 1e-3 \
    --backbone_lr 1e-4 \
    --lora_rank 8 \
    --lora_alpha 16 \
    --sensor_aux_loss_weight 0.4 \
    --use_wandb \
    --wandb_project query_dataset \
    --wandb_run_name "$LORAMOE_STAGE_B_RUN_NAME" &
PID_LORAMOE=$!

wait_jobs "$PID_MOE:moe_postblock" "$PID_LORA:qv_lora" "$PID_LORAMOE:qv_loramoe"
