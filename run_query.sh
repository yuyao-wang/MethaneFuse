...
if [[ "$QUERY_MODE" == "seg" || "$QUERY_MODE" == "all" ]]; then
  if [ ! -f "$UNIVERSAL_CKPT" ]; then
    found_ckpt="$(find "$UNIVERSAL_CHECKPOINT_DIR" -type f -name ckpt_best_test.pth | sort | tail -n 1 || true)"
    if [ -n "$found_ckpt" ]; then
      UNIVERSAL_CKPT="$found_ckpt"
    fi
  fi
  [ -f "$UNIVERSAL_CKPT" ] || { echo "Missing UNIVERSAL_CKPT: $UNIVERSAL_CKPT" >&2; exit 1; }
