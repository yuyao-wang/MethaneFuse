...
FIRST_DONE="$ROOT/Upgraded_dataset/s2_canonical_recrop_v4.done"
LOG="$ROOT/Upgraded_dataset/s2_canonical_local_retry_v4.log"
DONE="$ROOT/Upgraded_dataset/s2_canonical_local_retry_v4.done"
FAILED="$ROOT/Upgraded_dataset/s2_canonical_local_retry_v4.failed"

rm -f "$DONE" "$FAILED"
exec > >(tee -a "$LOG") 2>&1

echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] waiting for first recrop pass"
for _ in $(seq 1 720); do
  if [[ -e "$FIRST_DONE" ]]; then
    break
  fi
  sleep 10
done
if [[ ! -e "$FIRST_DONE" ]]; then
  echo "first recrop pass did not finish"
  touch "$FAILED"
  exit 1
fi

"$PYTHON" "$ROOT/Upgraded_dataset/s2_repair_failed_with_local_siblings.py"
...
timeout --signal=TERM --kill-after=5m 45m \
  ...
  --table "$ROOT/Upgraded_dataset/s2_canonical_6510_recrop_retry_local_v4.csv" \
  --output-table "$ROOT/Upgraded_dataset/s2_canonical_6510_recrop_retry_progress_v4.csv" \
...
"$PYTHON" -c "import pandas as pd; p='$ROOT/Upgraded_dataset/s2_canonical_6510_complete_v4.csv'; d=pd.read_csv(p); assert len(d)>=5300, len(d); print('complete_rows',len(d))"

printf 'done\n' > "$DONE"
echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] local sibling retry complete"
