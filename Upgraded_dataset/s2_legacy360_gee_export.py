#!/usr/bin/env python3
"""Submit matched legacy-three-time Sentinel-2 exports from Google Earth Engine.

This is a reproducible, resumable wrapper around the old GEE Sentinel-2 flow
in ``data_preprocess/gee_download_3_90_365_l2a.py``. It preserves its exact
event-relative date windows:

    t0:       [event day - 1, event day + 2)
    seasonal: [event day - 90, event day - 30)
    year:     [event day - 425, event day - 335)

It uses ``COPERNICUS/S2_SR_HARMONIZED`` and the same 12-band order:

    B1,B2,B3,B4,B5,B6,B7,B8,B8A,B9,B11,B12

The old t0 downloader exported every image in its three-day window. The
historical cohort table records which t0 acquisition was retained, so this
wrapper selects the acquisition closest to ``t0_image_time``. Seasonal and
year images retain the old first-image-after-NODATA-sort behavior.
"""
...
DEFAULT_DRIVE_FOLDER = "CM_S2_LEGACY360_MATCHED_GEE"
DEFAULT_RAW_ROOT = "/diniuvol/yuyao/s2_legacy360_matched_gee_raw"
...
LEGACY_TIMEPOINTS = {"t0", "seasonal", "year"}
CANONICAL_FILENAME = {
    "t0": "s2_0.tif",
    "prev1": "s2_prev1.tif",
    "prev2": "s2_prev2.tif",
...
def query_window(row: dict[str, Any], tp: Timepoint, args: argparse.Namespace) -> tuple[Optional[datetime], datetime, datetime, str]:
    del args
    event_time = parse_utc(row.get("event_time")) or parse_utc(row.get("datetime"))
    if event_time is None:
        raise ValueError("missing event_time/datetime")
    event_day = datetime(
        event_time.year,
        event_time.month,
        event_time.day,
    ...
    if tp.name == "t0":
        target = parse_utc(row.get(tp.image_time_col)) or event_time
        return (
            target,
            event_day - timedelta(days=1),
            event_day + timedelta(days=2),
            "legacy_t0_event_day_window",
        )
    if tp.name == "seasonal":
        return (
            None,
            event_day - timedelta(days=90),
            event_day - timedelta(days=30),
            "legacy_seasonal_event_day_window",
        )
    if tp.name == "year":
        return (
            None,
            event_day - timedelta(days=425),
            event_day - timedelta(days=335),
            "legacy_year_event_day_window",
        )
    raise ValueError(f"legacy exporter does not support timepoint={tp.name}")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Submit matched legacy-three-time GEE Sentinel-2 exports.")
    ...
    parser.add_argument("--timepoints", default="t0,seasonal,year", help="Comma list: t0,seasonal,year")
...
def main() -> int:
    ...
    unsupported = sorted({tp.name for tp in timepoints} - LEGACY_TIMEPOINTS)
    if unsupported:
        raise ValueError(
            f"legacy exporter supports only {sorted(LEGACY_TIMEPOINTS)}; "
