...
class RowGateHead(nn.Module):
    """Predict row-level gate logits for fused vs reference interpolation."""

    def __init__(self, *, embed_dim: int = 768, hidden_dim: int = 128, dropout: float = 0.1):
        super().__init__()
        self.net = nn.Sequential(
...
class MultiSensorPanopticonClassifier(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.row_gate_head: Optional[RowGateHead] = None
        ...
        if can_build_summary:
            ...
            if enable_row_gate:
                self.row_gate_head = RowGateHead(
                    embed_dim=embed_dim,
    ...
    def compute_row_fusion_bundle(
        ...
        *,
        ...
        fused_logits = self.row_fusion_head(row_features)
        bundle: Dict[str, torch.Tensor] = {
            "row_features": row_features,
            "fused_logits": fused_logits,
        }
        if self.row_gate_head is not None:
            gate_logits = self.row_gate_head(row_features)
            bundle["gate_logits"] = gate_logits
            bundle["gate"] = torch.sigmoid(gate_logits)
        return bundle
    ...
    def compute_row_fused_logits(
        ...
        *,
        ...
        return self.compute_row_fusion_bundle(
            outputs,
    ...
    torch.save(
        {
            ...
            "best_non_overlap_acc": best_non_overlap_acc,
            "args": vars(args),
        },
        path,
    )
...
def try_resume(path: Path, model: nn.Module, optimizer, scheduler, scaler, device):
    ...
    best_non_overlap_acc = ckpt.get("best_non_overlap_acc", float("-inf"))
    print(f"Resumed from {path} at epoch {start_epoch-1}", flush=True)
    return start_epoch, global_step, best_train_acc, best_test_acc, best_non_overlap_acc
...
def build_row_sensor_presence(
    ...
    *,
    ...
    row_has_sensor = torch.zeros((num_rows, len(sensor_order)), dtype=torch.bool, device=device)
    sensor_to_idx = {sensor: idx for idx, sensor in enumerate(sensor_order)}
    for sample_idx, sensor_name in enumerate(sensors):
        sensor_idx = sensor_to_idx.get(str(sensor_name))
        if sensor_idx is None:
            continue
        row_idx = int(sample_to_row[sample_idx].item())
        row_has_sensor[row_idx, sensor_idx] = True
    return row_has_sensor


def single_row_sensor_indices(row_has_sensor: torch.Tensor, single_row_ids: torch.Tensor) -> torch.Tensor:
    """Return sensor index for each single-row id (exactly one active sensor per row)."""
    if row_has_sensor.ndim != 2:
        raise ValueError(f"row_has_sensor must be rank-2, got shape={tuple(row_has_sensor.shape)}")
    if single_row_ids.ndim != 1:
        raise ValueError(f"single_row_ids must be rank-1, got shape={tuple(single_row_ids.shape)}")
    if single_row_ids.numel() == 0:
        return torch.empty((0,), dtype=torch.long, device=row_has_sensor.device)
    single_presence = row_has_sensor.index_select(0, single_row_ids).to(dtype=torch.int64)
    return torch.argmax(single_presence, dim=1)


def build_reference_row_logits(
    outputs: Dict[str, Union[Dict[str, torch.Tensor], torch.Tensor]],
    *,
    sample_to_row: torch.Tensor,
    num_rows: int,
    ...
    merged = outputs.get("merged_logits")
    if isinstance(merged, torch.Tensor):
        sample_logits = merged
    else:
        ...
        for sensor_out in outputs.values():
            ...
            if not (isinstance(idx, torch.Tensor) and isinstance(sensor_logits, torch.Tensor) and idx.numel() > 0):
                continue
            if sample_logits is None:
                sample_logits = sensor_logits.new_zeros((sample_to_row.shape[0], num_classes), device=device)
            sample_logits.index_copy_(0, idx.to(device=device, dtype=torch.long), sensor_logits.to(device=device))
        if sample_logits is None:
            sample_logits = torch.zeros((sample_to_row.shape[0], num_classes), device=device)

    row_ids, row_logits = mean_logits_by_row(sample_logits, sample_to_row, num_rows=num_rows)
    ref_logits = sample_logits.new_zeros((num_rows, num_classes), device=device)
    if row_ids.numel() > 0:
        ref_logits.index_copy_(0, row_ids, row_logits.to(device=device, dtype=ref_logits.dtype))
    return ref_logits
...
def gather_head_parameters(model: MultiSensorPanopticonClassifier) -> Sequence[nn.Parameter]:
    ...
    row_gate_params = list(model.row_gate_head.parameters()) if model.row_gate_head is not None else []
    head_params = list(model.heads.parameters()) + summary_params + row_fusion_params + row_gate_params + extra_patch_params
    return head_params
...
def parse_args():
    ...
    parser.add_argument("--row_gate", action="store_true", dest="row_gate")
    parser.add_argument("--disable_row_gate", action="store_false", dest="row_gate")
    parser.set_defaults(row_gate=True)
    ...
    parser.add_argument(
        "--non_overlap_kd_weight",
        type=float,
        default=0.5,
        help="KL distillation weight on non-overlap rows (student=final logits, teacher=reference row logits).",
    )
    parser.add_argument(
        "--gate_supervision_weight",
        type=float,
        default=0.1,
        help="BCE supervision weight for row gate (target: overlap=1, single=0).",
    )
    parser.add_argument(
        "--single_fused_ce_weight",
        type=float,
        default=0.5,
        help="Extra CE supervision weight on fused logits for non-overlap(single) rows.",
    )
    parser.add_argument(
        "--single_weight_s2",
        type=float,
        default=1.0,
        help="Single-row fused CE sample weight for S2 rows.",
    )
    parser.add_argument(
        "--single_weight_l89",
        type=float,
        default=1.0,
        help="Single-row fused CE sample weight for L89 rows.",
    )
    parser.add_argument(
        "--single_weight_s5p",
        type=float,
        default=1.2,
        help="Single-row fused CE sample weight for S5P rows.",
    )
    parser.add_argument(
        "--single_weight_wv3",
        type=float,
        default=2.0,
        help="Single-row fused CE sample weight for WV3 rows.",
    )
    parser.add_argument(
        "--best_non_overlap_tolerance",
        type=float,
        default=0.002,
        help="Allow best-checkpoint selection only when non-overlap acc is within this margin of historical best.",
    )
...
def main(args):
    ...
    best_non_overlap_acc = float("-inf")

    if args.resume:
        start_epoch, global_step, best_train_acc, best_test_acc, best_non_overlap_acc = try_resume(
    ...
    if args.non_overlap_kd_weight < 0.0:
        raise ValueError(f"--non_overlap_kd_weight must be >= 0, got {args.non_overlap_kd_weight}.")
    if args.gate_supervision_weight < 0.0:
        raise ValueError(f"--gate_supervision_weight must be >= 0, got {args.gate_supervision_weight}.")
    if args.single_fused_ce_weight < 0.0:
        raise ValueError(f"--single_fused_ce_weight must be >= 0, got {args.single_fused_ce_weight}.")
    if args.single_weight_s2 < 0.0:
        raise ValueError(f"--single_weight_s2 must be >= 0, got {args.single_weight_s2}.")
    if args.single_weight_l89 < 0.0:
        raise ValueError(f"--single_weight_l89 must be >= 0, got {args.single_weight_l89}.")
    if args.single_weight_s5p < 0.0:
        raise ValueError(f"--single_weight_s5p must be >= 0, got {args.single_weight_s5p}.")
    if args.single_weight_wv3 < 0.0:
        raise ValueError(f"--single_weight_wv3 must be >= 0, got {args.single_weight_wv3}.")
    if args.best_non_overlap_tolerance < 0.0:
        raise ValueError(f"--best_non_overlap_tolerance must be >= 0, got {args.best_non_overlap_tolerance}.")
    single_sensor_weight_map = {
        "s2": float(args.single_weight_s2),
        "l89": float(args.single_weight_l89),
        "s5p": float(args.single_weight_s5p),
        "wv3": float(args.single_weight_wv3),
    }
    single_sensor_weight_vector = torch.tensor(
        [single_sensor_weight_map.get(sensor, 1.0) for sensor in sensors_list],
        dtype=torch.float32,
    )
    ...
    print(
        ...
        f"row_gate={int(args.row_gate)}, non_overlap_kd_weight={args.non_overlap_kd_weight:.3f}, "
        f"gate_supervision_weight={args.gate_supervision_weight:.3f}, "
        f"single_fused_ce_weight={args.single_fused_ce_weight:.3f}, "
        f"single_sensor_weights="
        + ",".join(
            f"{sensor}:{single_sensor_weight_map.get(sensor, 1.0):.2f}" for sensor in sensors_list
        ),
        flush=True,
    )
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        if core_model.row_gate_head is not None:
            core_model.row_gate_head.train()
        ...
        gate_loss_accum = 0.0
        gate_loss_count = 0
        single_fused_ce_loss_accum = 0.0
        single_fused_ce_loss_count = 0
        single_weight_mean_accum = 0.0
        single_weight_mean_count = 0
        gate_mean_accum = 0.0
        gate_mean_count = 0
        ...
        for x_dict, labels, sensors, sample_to_row in train_loader:
            ...
            with autocast(enabled=use_amp):
                ...
                fusion_bundle = core_model.compute_row_fusion_bundle(
                    outputs,
                    sample_to_row=sample_to_row,
                    num_rows=labels.size(0),
                    device=device,
                )
                fused_logits = fusion_bundle["fused_logits"]
                gate_values = fusion_bundle.get("gate")
                gate_logits = fusion_bundle.get("gate_logits")
                num_classes = int(fused_logits.shape[-1])
                ref_row_logits = build_reference_row_logits(
                    outputs,
                    sample_to_row=sample_to_row,
                    num_rows=labels.size(0),
                ...
                if isinstance(gate_values, torch.Tensor):
                    gate_expand = gate_values.unsqueeze(-1)
                    final_logits = (gate_expand * fused_logits) + ((1.0 - gate_expand) * ref_row_logits)
                else:
                    final_logits = fused_logits

                row_has_sensor = build_row_sensor_presence(
                    sensors,
                    sample_to_row,
                    num_rows=labels.size(0),
                    sensor_order=sensors_list,
                    device=device,
                )
                row_sensor_count = row_has_sensor.sum(dim=1)
                overlap_mask = row_sensor_count >= 2
                non_overlap_mask = row_sensor_count == 1

                fused_loss = criterion(final_logits, labels)
                if args.non_overlap_kd_weight > 0.0 and torch.any(non_overlap_mask):
                    kd_loss = kl_distill_loss(
                        final_logits[non_overlap_mask],
                        ref_row_logits[non_overlap_mask],
                    )
                else:
                    kd_loss = fused_loss.new_zeros(())

                if isinstance(gate_logits, torch.Tensor) and args.gate_supervision_weight > 0.0:
                    gate_targets = overlap_mask.to(dtype=gate_logits.dtype)
                    gate_loss = F.binary_cross_entropy_with_logits(gate_logits, gate_targets)
                else:
                    gate_loss = fused_loss.new_zeros(())
                if args.single_fused_ce_weight > 0.0 and torch.any(non_overlap_mask):
                    single_row_ids = torch.nonzero(non_overlap_mask, as_tuple=False).squeeze(1)
                    single_logits = fused_logits.index_select(0, single_row_ids)
                    single_labels = labels.index_select(0, single_row_ids)
                    ...
                    single_sensor_idx = single_row_sensor_indices(row_has_sensor, single_row_ids)
                    weight_vec = single_sensor_weight_vector.to(device=device, dtype=single_ce.dtype)
                    single_weights = weight_vec.index_select(0, single_sensor_idx).clamp_min(0.0)
                    weighted_single_sum = torch.sum(single_ce * single_weights)
                    single_fused_ce_loss = weighted_single_sum / single_weights.sum().clamp_min(1e-6)
                    single_weight_mean = single_weights.mean()
                else:
                    single_fused_ce_loss = fused_loss.new_zeros(())
                    single_weight_mean = fused_loss.new_zeros(())

                loss = (
                    fused_loss
                    + (args.sensor_aux_loss_weight * sensor_aux_loss)
                    + (args.non_overlap_kd_weight * kd_loss)
                    + (args.gate_supervision_weight * gate_loss)
                    + (args.single_fused_ce_weight * single_fused_ce_loss)
                )
            optimizer.zero_grad()
            scaler.scale(loss).backward()
            if args.max_grad_norm and args.max_grad_norm > 0:
                scaler.unscale_(optimizer)
                torch.nn.utils.clip_grad_norm_((p for p in core_model.parameters() if p.requires_grad), args.max_grad_norm)
            ...
            kd_loss_accum += kd_loss.item()
            kd_loss_count += 1
            gate_loss_accum += gate_loss.item()
            gate_loss_count += 1
            single_fused_ce_loss_accum += single_fused_ce_loss.item()
            single_fused_ce_loss_count += 1
            single_weight_mean_accum += single_weight_mean.item()
            single_weight_mean_count += 1
            if isinstance(gate_values, torch.Tensor):
                gate_mean_accum += gate_values.mean().item()
                gate_mean_count += 1
            ...
            if args.log_interval and step_idx % args.log_interval == 0:
                ...
                sensor_loss_details = (
                    f"fused_loss={fused_loss.item():.4f} sensor_aux_loss={sensor_aux_loss.item():.4f} "
                    f"kd_loss={kd_loss.item():.4f} gate_loss={gate_loss.item():.4f} "
                    f"single_fused_ce_loss={single_fused_ce_loss.item():.4f} "
                    f"single_weight_mean={single_weight_mean.item():.4f} "
                    f"gate_mean={(gate_values.mean().item() if isinstance(gate_values, torch.Tensor) else float('nan')):.4f} "
                    f"{sensor_loss_details}"
                ).strip()
                print(
        ...
        ref_row_all_labels: list[int] = []
        ref_row_all_preds: list[int] = []
        ref_row_all_pos_scores: list[float] = []
        ref_row_overlap_labels: list[int] = []
        ref_row_overlap_preds: list[int] = []
        ref_row_overlap_pos_scores: list[float] = []
        ref_row_single_labels: list[int] = []
        ref_row_single_preds: list[int] = []
        ref_row_single_pos_scores: list[float] = []
        eval_gate_all: list[float] = []
        eval_gate_overlap: list[float] = []
        eval_gate_single: list[float] = []
        ...
        with torch.no_grad():
            for step, (x_dict, labels, sensors, sample_to_row) in enumerate(test_loader, 1):
                ...
                if final_logits.shape[-1] == 2:
                    pos_scores = torch.softmax(final_logits, dim=1)[:, 1]
                ref_preds = ref_row_logits.argmax(dim=1)
                ref_pos_scores: Optional[torch.Tensor] = None
                if ref_row_logits.shape[-1] == 2:
                    ref_pos_scores = torch.softmax(ref_row_logits, dim=1)[:, 1]
                ...
                if isinstance(gate_values, torch.Tensor):
                    eval_gate_all.extend(gate_values.detach().to("cpu").tolist())
                    if torch.any(overlap_mask):
                        eval_gate_overlap.extend(
                            gate_values.index_select(0, torch.nonzero(overlap_mask, as_tuple=False).squeeze(1))
                            .detach()
                            .to("cpu")
                            .tolist()
                        )
                    if torch.any(single_mask):
                        eval_gate_single.extend(
                            gate_values.index_select(0, torch.nonzero(single_mask, as_tuple=False).squeeze(1))
                            .detach()
                            .to("cpu")
                            .tolist()
                        )
                ...
                def _append_rows(
                    mask: torch.Tensor,
                    row_preds: torch.Tensor,
                    row_scores: Optional[torch.Tensor],
                    label_bucket: list[int],
                    pred_bucket: list[int],
                    ...
                    pred_bucket.extend(row_preds.index_select(0, row_ids).detach().to("cpu").tolist())
                    if row_scores is not None:
                        score_bucket.extend(row_scores.index_select(0, row_ids).detach().to("cpu").tolist())

                all_mask = torch.ones((batch_rows,), dtype=torch.bool, device=device)
                _append_rows(all_mask, preds, pos_scores, fused_all_labels, fused_all_preds, fused_all_pos_scores)
                _append_rows(overlap_mask, preds, pos_scores, fused_overlap_labels, fused_overlap_preds, fused_overlap_pos_scores)
                _append_rows(single_mask, preds, pos_scores, fused_single_labels, fused_single_preds, fused_single_pos_scores)
                _append_rows(all_mask, ref_preds, ref_pos_scores, ref_row_all_labels, ref_row_all_preds, ref_row_all_pos_scores)
                _append_rows(
                    overlap_mask,
                    ref_preds,
                    ref_pos_scores,
                    ref_row_overlap_labels,
                    ref_row_overlap_preds,
                    ref_row_overlap_pos_scores,
                )
                _append_rows(
                    single_mask,
                    ref_preds,
                    ref_pos_scores,
                    ref_row_single_labels,
                    ref_row_single_preds,
                    ref_row_single_pos_scores,
                )
        ...
        ref_row_all_overall_metrics = compute_split_metrics(
            ref_row_all_labels,
            ref_row_all_preds,
            ref_row_all_pos_scores,
        )
        ref_row_overlap_overall_metrics = compute_split_metrics(
            ref_row_overlap_labels,
            ref_row_overlap_preds,
            ref_row_overlap_pos_scores,
        )
        ref_row_single_overall_metrics = compute_split_metrics(
            ref_row_single_labels,
            ref_row_single_preds,
            ref_row_single_pos_scores,
        )
        gate_mean_all = float(sum(eval_gate_all) / len(eval_gate_all)) if eval_gate_all else float("nan")
        gate_mean_overlap = float(sum(eval_gate_overlap) / len(eval_gate_overlap)) if eval_gate_overlap else float("nan")
        gate_mean_single = float(sum(eval_gate_single) / len(eval_gate_single)) if eval_gate_single else float("nan")
        ...
        print(
            ...
            f"train_kd_loss={kd_loss_accum/max(1,kd_loss_count):.4f} "
            f"train_gate_loss={gate_loss_accum/max(1,gate_loss_count):.4f} "
            f"train_single_fused_ce_loss={single_fused_ce_loss_accum/max(1,single_fused_ce_loss_count):.4f} "
            f"train_single_weight_mean={single_weight_mean_accum/max(1,single_weight_mean_count):.4f} "
            f"train_gate_mean={gate_mean_accum/max(1,gate_mean_count):.4f} "
            f"eval_gate_mean_all={gate_mean_all:.4f} eval_gate_mean_overlap={gate_mean_overlap:.4f} "
            f"eval_gate_mean_single={gate_mean_single:.4f}",
            flush=True,
        )
        print(
        ...
        print(
            f"[EvalSplit][REF_ROW][ALL][OVERALL] count={int(ref_row_all_overall_metrics['count'])} "
            f"acc={ref_row_all_overall_metrics['acc']:.4f} fpr={ref_row_all_overall_metrics['fpr']:.4f} "
            f"recall={ref_row_all_overall_metrics['recall']:.4f} auroc={ref_row_all_overall_metrics['auroc']:.4f}",
            flush=True,
        )
        print(
            f"[EvalSplit][REF_ROW][OVERLAP][OVERALL] count={int(ref_row_overlap_overall_metrics['count'])} "
            f"acc={ref_row_overlap_overall_metrics['acc']:.4f} fpr={ref_row_overlap_overall_metrics['fpr']:.4f} "
            f"recall={ref_row_overlap_overall_metrics['recall']:.4f} auroc={ref_row_overlap_overall_metrics['auroc']:.4f}",
            flush=True,
        )
        print(
            f"[EvalSplit][REF_ROW][SINGLE][OVERALL] count={int(ref_row_single_overall_metrics['count'])} "
            f"acc={ref_row_single_overall_metrics['acc']:.4f} fpr={ref_row_single_overall_metrics['fpr']:.4f} "
            f"recall={ref_row_single_overall_metrics['recall']:.4f} auroc={ref_row_single_overall_metrics['auroc']:.4f}",
            flush=True,
        )
        for sensor in sensors_list:
        ...
        prev_best_non_overlap = best_non_overlap_acc
        best_train_acc = max(best_train_acc, train_acc)
        current_non_overlap_acc = float(fused_single_overall_metrics["acc"])
        if math.isfinite(current_non_overlap_acc):
            best_non_overlap_acc = max(best_non_overlap_acc, current_non_overlap_acc)
        ...
        save_checkpoint(
            ...
            best_train_acc,
            best_test_acc,
            best_non_overlap_acc,
            args,
        )
        non_overlap_ok = (
            (not math.isfinite(prev_best_non_overlap))
            or (current_non_overlap_acc >= (prev_best_non_overlap - args.best_non_overlap_tolerance))
        )
        if test_acc > prev_best_test and non_overlap_ok:
            ...
            print(
                ...
                f"(test_acc={test_acc:.4f}, non_overlap_acc={current_non_overlap_acc:.4f})",
                flush=True,
            )
        elif test_acc > prev_best_test and (not non_overlap_ok):
            print(
                "Best-test candidate skipped by non-overlap guard: "
                f"test_acc={test_acc:.4f}, non_overlap_acc={current_non_overlap_acc:.4f}, "
                f"required>={prev_best_non_overlap - args.best_non_overlap_tolerance:.4f}",
                flush=True,
            )
        if wandb_run is not None:
            log_payload = {
                ...
                "train_kd_loss": kd_loss_accum / max(1, kd_loss_count),
                "train_gate_loss": gate_loss_accum / max(1, gate_loss_count),
                "train_single_fused_ce_loss": single_fused_ce_loss_accum / max(1, single_fused_ce_loss_count),
                "train_single_weight_mean": single_weight_mean_accum / max(1, single_weight_mean_count),
                "train_gate_mean": gate_mean_accum / max(1, gate_mean_count),
                "eval_gate_mean_all": gate_mean_all,
                "eval_gate_mean_overlap": gate_mean_overlap,
                "eval_gate_mean_single": gate_mean_single,
                "test_ref_row_all_overall_count": ref_row_all_overall_metrics["count"],
                "test_ref_row_all_overall_acc": ref_row_all_overall_metrics["acc"],
                "test_ref_row_all_overall_fpr": ref_row_all_overall_metrics["fpr"],
                "test_ref_row_all_overall_recall": ref_row_all_overall_metrics["recall"],
                "test_ref_row_all_overall_auroc": ref_row_all_overall_metrics["auroc"],
                "test_ref_row_overlap_overall_count": ref_row_overlap_overall_metrics["count"],
                "test_ref_row_overlap_overall_acc": ref_row_overlap_overall_metrics["acc"],
                "test_ref_row_overlap_overall_fpr": ref_row_overlap_overall_metrics["fpr"],
                "test_ref_row_overlap_overall_recall": ref_row_overlap_overall_metrics["recall"],
                "test_ref_row_overlap_overall_auroc": ref_row_overlap_overall_metrics["auroc"],
                "test_ref_row_single_overall_count": ref_row_single_overall_metrics["count"],
                "test_ref_row_single_overall_acc": ref_row_single_overall_metrics["acc"],
                "test_ref_row_single_overall_fpr": ref_row_single_overall_metrics["fpr"],
                "test_ref_row_single_overall_recall": ref_row_single_overall_metrics["recall"],
                "test_ref_row_single_overall_auroc": ref_row_single_overall_metrics["auroc"],
            }
            for sensor in sensors_list:
