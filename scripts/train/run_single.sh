...
OUT_ROOT=${OUT_ROOT:-/transferdiniu2/yuyao/checkpoints/480m_518_single4_retrain_${TS}}
...
# Train all single-sensor models on CUDA 0.
run_train s2 "$PYTHON_BIN" universal_models_fusion/dino_clssifier_head_s2_temportal_one_block.py \
  --train_csv "$TRAIN_CSV" --test_csv "$TEST_CSV" --weights "$WEIGHTS" \
  --device cuda --train_backbone --epochs 2 --batch_size 8 --num_workers 6 \
...
run_train l89 "$PYTHON_BIN" universal_models_fusion/dino_clssifier_head_l89_temportal_one_block.py \
  --train_csv "$TRAIN_CSV" --test_csv "$TEST_CSV" --weights "$WEIGHTS" \
  --device cuda --train_backbone --epochs 7 --batch_size 8 --num_workers 6 \
...
run_train s5p "$PYTHON_BIN" universal_models_fusion/dino_classifier_head_s5p_temporal_one_block.py \
  --train_csv "$TRAIN_CSV" --test_csv "$TEST_CSV" --weights "$WEIGHTS" \
  --device cuda --train_backbone --epochs 7 --batch_size 8 --num_workers 6 \
  --head_lr 5e-4 --backbone_lr 5e-5 --lr_scheduler none \
...
run_train wv3 "$PYTHON_BIN" universal_models_fusion/dino_clssifier_head_EMIT_simulated_wv3_temporal_one_block.py \
  --train_csv "$TRAIN_CSV" --test_csv "$TEST_CSV" --weights "$WEIGHTS" \
  --device cuda --train_backbone --epochs 7 --batch_size 8 --num_workers 6 \
  --head_lr 5e-4 --backbone_lr 5e-5 --lr_scheduler none \
...
"$PYTHON_BIN" universal_models_fusion/infer_overlap_or_single_models_native.py \
  ...
  --batch_size 8 --sensor_sub_batch_size 2 --model_resident one_by_one --amp_dtype none --num_workers 8 \
  --s2_ckpt "$S2_CKPT" \
  --l89_ckpt "$L89_CKPT" \
  --s5p_ckpt "$S5P_CKPT" \
  --wv3_ckpt "$WV3_CKPT" \
