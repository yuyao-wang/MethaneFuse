...
def main(args):
    ...
    train_ds = S5pTemporalNpzDataset(
    ...
    resolved_stats = train_ds.get_normalize_stats()
    if resolved_stats is None:
        resolved_stats = norm_stats
    elif norm_stats is None:
        mean, std = resolved_stats
        print(f"Computed normalization stats from training set (mean len={len(mean)}, std len={len(std)})", flush=True)

    test_ds = S5pTemporalNpzDataset(
    ...
    stats_status = train_ds.get_stats_source()
    if stats_status is None:
        stats_status = "provided" if resolved_stats is not None else "none"
    print(
        f"Using device={device}, fusion={args.fusion}, train_samples={len(train_ds)}, test_samples={len(test_ds)}, norm_stats={stats_status}",
        flush=True,
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Temporal finetuning of Panopticon backbone plus MLP head for Sentinel-5P NPZ patches.")
    ...
    parser.add_argument("--ds_cfg_name", default=None, help="Optional dataset config name for channel IDs (under dinov2/configs/data).")
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Constant channel id used when none are provided (keeps model happy for single-channel S5P CH4 grids).",
    )
    parser.add_argument(
        "--stacked_time_npz",
        action="store_true",
