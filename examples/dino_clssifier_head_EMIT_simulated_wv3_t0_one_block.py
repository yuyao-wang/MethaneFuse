...
def main(args):
    ...
    train_ds.chn_ids = wv3_chn_ids
    test_ds.chn_ids = wv3_chn_ids.clone()
    wv3_mus = train_ds.chn_ids[:, 0] if train_ds.chn_ids.ndim == 2 else train_ds.chn_ids
    print(
        f"WV3 channel IDs loaded from {args.wv3_srf_csv}: bands={len(wv3_mus)}, "
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Panopticon ViT + CLS head with EMIT-simulated WV3 t0 inputs."
    )
