...
class TriSensorTemporalCsvDataset(S2TemporalCsvDataset):
    ...
    def __getitem__(self, idx):
        ...
        while attempts < self.max_retries:
            try:
                ...
                if sensor == "s5p":
                    x_dict = self._load_s5p_sample(row, self.path_columns[0], sample_id=idx)
                ...
                attempts += 1
                if attempts == 1:
                    warnings.warn(f"Skipping invalid sample at temporal index {idx}: {exc}. Retrying with a different index.")
    ...
    def _load_s5p_sample(self, row, column_name: str, sample_id: Optional[int] = None) -> Dict[str, torch.Tensor]:
        ...
        if not os.path.isfile(path):
            raise _SkipSample(f"S5P sample file not found: {path}")
        try:
            np_obj = np.load(path, allow_pickle=False)
        except (FileNotFoundError, OSError, ValueError, EOFError, zipfile.BadZipFile) as exc:
            context = f"sample={sample_id}, column={column_name}" if sample_id is not None else f"column={column_name}"
            raise _SkipSample(f"S5P load failed for {path} ({context}): {exc}") from exc
        chn_ids = None
        try:
            ...
            raise _SkipSample(f"S5P NPZ payload invalid for {path}: {exc}") from exc
        finally:
            if isinstance(np_obj, np.lib.npyio.NpzFile):
                np_obj.close()
        if img_np.ndim == 2:
            ...
            raise _SkipSample(f"S5P sample has unsupported shape {img_np.shape} at {path}")
        ...
        if img.shape[0] != expected_channels:
            ...
            elif expected_channels % img.shape[0] == 0:
                img = img.repeat(expected_channels // img.shape[0], 1, 1)
            else:
                raise _SkipSample(
                    f"S5P channel mismatch for {path}: expected {expected_channels}, got {img.shape[0]}"
                )
        ...
        if chn_ids.shape[0] != img.shape[0]:
            if chn_ids.shape[0] == 1 and img.shape[0] > 1:
                chn_ids = chn_ids.repeat(img.shape[0], 1)
            else:
                raise _SkipSample(
                    f"S5P chn_ids mismatch for {path}: imgs has {img.shape[0]} channels but chn_ids has {chn_ids.shape[0]}"
                )
...
def main(args):
    ...
    print(
        ...
        f"adapter_blocks={len(adapter_blocks)}, freeze_vit_in_adapter_blocks={args.freeze_vit_in_adapter_blocks}",
        flush=True,
    )
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        for block in adapter_blocks:
            if isinstance(block, SensorAdapterBlock):
                block.adapters.train(train_adapters)
