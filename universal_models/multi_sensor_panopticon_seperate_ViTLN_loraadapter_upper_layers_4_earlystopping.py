"""Multi-sensor Panopticon training script with sensor-specific embeddings and one shared head.
...
Panopticon patch embedding, while the DinoViT backbone and classifier head are
...
from typing import Any, Callable, Dict, Iterator, Mapping, MutableMapping, Optional, Protocol, Sequence, Tuple, Union
...
class MultiSensorPanopticonClassifier(nn.Module):
    """Shared DinoViT backbone with sensor-specific Panopticon PEs and one universal head."""
    ...
    def __init__(
        ...
        *,
        ...
        enable_summary_head: bool = False,
        ...
        if isinstance(num_classes, int):
            num_out_classes = int(num_classes)
        else:
            class_values = [int(num_classes[sensor]) for sensor in self.sensor_order]
            if len(set(class_values)) != 1:
                raise ValueError("Universal head requires the same num_classes for all sensors.")
            num_out_classes = class_values[0]
    ...
    def _set_backbone_domain_id(self, domain_id: torch.Tensor) -> None:
        # DataParallel replicas shallow-copy plain python attributes. Do not rely on
        # backbone._sensor_adapter_blocks (a plain list), otherwise replicas can race
        # on shared references. Resolve adapter blocks from registered modules each call.
        ...
        *,
        ...
        for sensor_name, sensor_batch in sensor_batches.items():
            ...
            if self.summary_head is not None:
                ...
                summary_inputs.index_copy_(0, sensor_batch.indices, torch.cat([logits] * len(self.sensor_order), dim=-1))
            outputs[sensor_name] = {
    ...
    def _ensure_sensor_keys(
        ...
        *,
        ...
        num_classes = next(
            sensor_out["logits"].shape[-1]
            for sensor_out in outputs.values()
            if isinstance(sensor_out, dict) and "logits" in sensor_out
        )
        for sensor_name in self.sensor_order:
...
class StaticAnchoredCache:
    ...
    def cached_path(self, original: str) -> Optional[str]:
...
class TieredStaticAnchoredCache:
    """
    Multi-tier local cache:
    1) Reuse any already-cached file from any tier.
    2) On miss, try to cache in tier order.
    """

    def __init__(self, tiers: Sequence[StaticAnchoredCache]):
        self.tiers = list(tiers)
        if not self.tiers:
            raise ValueError("TieredStaticAnchoredCache requires at least one cache tier.")

    def ensure_local(self, original: str) -> str:
        for cache in self.tiers:
            hit = cache.cached_path(original)
            if hit is not None:
                return hit
        for cache in self.tiers:
            cached = cache.ensure_local(original)
            if cached != original:
                return cached
        return original
    ...
    def warm_up(self, paths: Sequence[str], max_workers: int = 8) -> None:
        ...
        if not unique_paths:
            return
        print(
            f"[Cache] Warming up tiered cache (tiers={len(self.tiers)}, target={len(unique_paths)})...",
            flush=True,
        )
...
class TriSensorTemporalCsvDataset(S2TemporalCsvDataset):
    ...
    def __init__(
        self,
        *args,
        local_file_cache: Optional[LocalFileCache] = None,
...
class MultiSensorTrainAugment:
    """Lightweight spatial/noise augmentations for multi-sensor training."""
    ...
    def __init__(
        ...
        *,
        ...
        erase_max_ratio: float = 0.2,
    ):
        self.hflip_prob = max(0.0, min(1.0, float(hflip_prob)))
        self.vflip_prob = max(0.0, min(1.0, float(vflip_prob)))
        self.rotate_prob = max(0.0, min(1.0, float(rotate_prob)))
        self.noise_prob = max(0.0, min(1.0, float(noise_prob)))
        self.noise_std = max(0.0, float(noise_std))
        self.erase_prob = max(0.0, min(1.0, float(erase_prob)))
        self.erase_max_ratio = max(0.01, min(0.95, float(erase_max_ratio)))

    def __call__(self, x_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        imgs = x_dict.get("imgs")
        if not torch.is_tensor(imgs) or imgs.ndim != 3:
            return x_dict
        ...
        if self.noise_std > 0 and self.noise_prob > 0 and random.random() < self.noise_prob:
            noise_scale = max(float(out.detach().float().std().item()), 1e-6) * self.noise_std
            out = out + torch.randn_like(out) * noise_scale

        if self.erase_prob > 0 and random.random() < self.erase_prob:
            c, h, w = out.shape
            erase_h = max(1, int(round(h * random.uniform(0.1, self.erase_max_ratio))))
            erase_w = max(1, int(round(w * random.uniform(0.1, self.erase_max_ratio))))
...
def build_train_augmentation(args):
    ...
    return MultiSensorTrainAugment(
        ...
        erase_max_ratio=args.aug_erase_max_ratio,
    )


def _best_threshold_for_accuracy(
    labels: np.ndarray,
    scores: np.ndarray,
    *,
    ...
    tp = 0
    fp = 0
    best_acc = n_neg / max(1, n)  # threshold above max(score): predict all negative
...
def calibrate_sensor_thresholds(
    *,
    ...
    thresholds = {sensor: float(default_threshold) for sensor in sensor_order}
    fit_acc = {sensor: float("nan") for sensor in sensor_order}
    for sensor in sensor_order:
        mask = sensors == sensor
        count = int(mask.sum())
        if count < max(1, int(min_samples)):
            continue
        th, acc = _best_threshold_for_accuracy(
            labels[mask],
            scores[mask],
        ...
        fit_acc[sensor] = acc
    return thresholds, fit_acc


def apply_sensor_thresholds(
    *,
    scores: np.ndarray,
    sensors: np.ndarray,
    ...
    assigned = np.zeros(scores.shape[0], dtype=bool)
    for sensor, threshold in thresholds.items():
        mask = sensors == sensor
        if not np.any(mask):
            continue
        preds[mask] = (scores[mask] >= float(threshold)).astype(np.int64)
        assigned[mask] = True
    if not np.all(assigned):
        fallback = float(default_threshold)
        preds[~assigned] = (scores[~assigned] >= fallback).astype(np.int64)
    return preds


def compute_per_sensor_accuracy(
    *,
    labels: np.ndarray,
    preds: np.ndarray,
    sensors: np.ndarray,
    ...
    for sensor in sensor_order:
        ...
        n = int(mask.sum())
        if n == 0:
            out[sensor] = float("nan")
            continue
        out[sensor] = float((preds[mask] == labels[mask]).sum() / n)
    return out


def _finite_macro_and_min(metric_map: Mapping[str, float], sensor_order: Sequence[str]) -> Tuple[float, float]:
    values = [float(metric_map[s]) for s in sensor_order if s in metric_map and math.isfinite(float(metric_map[s]))]
    if not values:
...
def compute_early_stopping_score(
    *,
    ...
    test_loss: float,
    test_acc: float,
    test_acc_calibrated: float,
    per_sensor_acc: Mapping[str, float],
    per_sensor_acc_calibrated: Mapping[str, float],
    sensor_order: Sequence[str],
    macro_weight: float,
    min_sensor_weight: float,
) -> Tuple[float, Dict[str, float]]:
    macro_raw, min_raw = _finite_macro_and_min(per_sensor_acc, sensor_order)
    macro_cal, min_cal = _finite_macro_and_min(per_sensor_acc_calibrated, sensor_order)
    if not math.isfinite(test_acc_calibrated):
        test_acc_calibrated = test_acc
    if not math.isfinite(macro_cal):
        macro_cal = macro_raw
    if not math.isfinite(min_cal):
        min_cal = min_raw
    ...
    if metric_name == "test_loss":
        return -float(test_loss), details
    if metric_name == "test_acc":
        return float(test_acc), details
    if metric_name == "test_acc_calibrated":
        return float(test_acc_calibrated), details
    if metric_name == "macro_sensor_acc":
        return float(macro_raw), details
    if metric_name == "macro_sensor_acc_calibrated":
        return float(macro_cal), details

    # Composite score to stabilize early stopping for a shared multi-sensor model:
    # overall performance + macro fairness + weakest-sensor floor.
    w_macro = max(0.0, float(macro_weight))
    w_min = max(0.0, float(min_sensor_weight))
    w_global = max(0.0, 1.0 - w_macro - w_min)
    denom = w_global + w_macro + w_min
    if denom <= 0:
        w_global = 1.0
        denom = 1.0
    score = (
        w_global * float(test_acc_calibrated)
        + w_macro * float(macro_cal)
        + w_min * float(min_cal)
    ) / denom
    details["composite"] = float(score)
...
def select_trainable_adapter_blocks(
    blocks: Sequence[nn.Module],
    train_blocks: Optional[int],
) -> list[SensorAdapterBlock]:
    available = [b for b in blocks if isinstance(b, SensorAdapterBlock)]
    if train_blocks is None:
        return available
    if train_blocks < 0:
        raise ValueError("--adapter_train_blocks must be >= 0")
    if train_blocks == 0:
        return []
    if train_blocks >= len(available):
        return available
    return available[:train_blocks]
...
def gather_head_parameters(model: MultiSensorPanopticonClassifier) -> Sequence[nn.Parameter]:
    ...
    params: list[nn.Parameter] = list(model.head.parameters())
...
def build_optimizer_param_groups(
    ...
    *,
    ...
    trainable_adapter_blocks: Optional[Sequence[nn.Module]] = None,
) -> list[dict]:
    ...
    shared_patch = model.backbone.patch_embed
    _add_group(list(model.head.parameters()), args.head_lr)
    for sensor_name in model.sensor_order:
    ...
    if args.train_backbone:
        if trainable_adapter_blocks is None:
            adapter_params = list(gather_adapter_parameters(model))
        else:
            adapter_params = list(_gather_adapter_parameters_from_blocks(trainable_adapter_blocks))
...
def parse_args():
    ...
    parser.add_argument(
        "--data_parallel_num_gpus",
        type=int,
        default=0,
        help="If >0, limit DataParallel to the first N visible GPUs.",
    )
    ...
    parser.add_argument(
        "--local_cache_fallback_dir",
        default=None,
        help="Secondary local cache dir. Files are reused from both dirs and new cache writes fall back here.",
    )
    ...
    parser.add_argument(
        "--local_cache_fallback_min_free_gb",
        type=float,
        default=None,
        help="Min free space for fallback cache dir (defaults to --local_cache_min_free_gb).",
    )
    parser.add_argument(
        "--local_cache_fallback_max_gb",
        type=float,
        default=None,
        help="Max size for fallback cache dir (defaults to --local_cache_max_gb).",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Comma-separated per-sensor epoch ratio, e.g. s2=1.0,l89=0.35,s5p=1.0,wv3=1.0",
    )
    parser.add_argument(
        "--sensor_head_lr_mult",
        type=str,
        default="",
        help="Comma-separated per-sensor patch-embed LR multiplier, e.g. s2=1.2,l89=0.5,s5p=1.0,wv3=1.0",
    )
    ...
    parser.add_argument("--train_augment", action="store_true", dest="train_augment")
    parser.add_argument("--disable_train_augment", action="store_false", dest="train_augment")
    ...
    parser.add_argument("--aug_noise_prob", type=float, default=0.3)
    parser.add_argument("--aug_noise_std", type=float, default=0.02)
    parser.add_argument("--aug_erase_prob", type=float, default=0.2)
    parser.add_argument("--aug_erase_max_ratio", type=float, default=0.2)
    parser.add_argument("--auto_sensor_thresholds", action="store_true", dest="auto_sensor_thresholds")
    parser.add_argument("--disable_auto_sensor_thresholds", action="store_false", dest="auto_sensor_thresholds")
    parser.set_defaults(auto_sensor_thresholds=True)
    parser.add_argument("--sensor_threshold_min_samples", type=int, default=128)
    parser.add_argument("--sensor_threshold_ema", type=float, default=0.0)
    parser.add_argument("--default_sensor_threshold", type=float, default=0.5)
    parser.add_argument("--early_stopping", action="store_true")
    parser.add_argument("--early_stopping_patience", type=int, default=6)
    parser.add_argument("--early_stopping_warmup_epochs", type=int, default=5)
    ...
    parser.add_argument(
        ...
        choices=[
            ...
            "test_acc_calibrated",
            "macro_sensor_acc",
            "macro_sensor_acc_calibrated",
        ],
        default="composite",
    )
    parser.add_argument("--early_stopping_macro_weight", type=float, default=0.4)
    parser.add_argument("--early_stopping_min_sensor_weight", type=float, default=0.1)
    parser.add_argument("--save_per_sensor_best", action="store_true", dest="save_per_sensor_best")
    parser.add_argument("--disable_save_per_sensor_best", action="store_false", dest="save_per_sensor_best")
    parser.set_defaults(save_per_sensor_best=True)
    parser.add_argument(
        "--adapter_first_blocks",
        type=int,
        default=5,
        help="Attach sensor adapters to the first N transformer blocks.",
    )
    parser.add_argument("--adapter_last_blocks", dest="adapter_first_blocks", type=int, help=argparse.SUPPRESS)
    parser.add_argument(
        "--adapter_train_blocks",
        type=int,
        default=None,
        help="Train only the first N adapterized blocks (default: train all adapterized blocks).",
    )
    parser.add_argument(
        ...
        type=int,
        default=16,
        help="Low-rank dimension (LoRA rank) for sensor adapters.",
    )
    parser.add_argument(
        "--adapter_alpha",
        "--lora_alpha",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Freeze base ViT weights inside adapterized blocks.",
    )
...
def main(args):
    ...
    cache_obj: Optional[LocalFileCache] = None
    if args.local_cache_dir:
        primary_cache = StaticAnchoredCache(
            args.local_cache_dir,
            min_free_gb=args.local_cache_min_free_gb,
            max_cache_gb=args.local_cache_max_gb,
        )
        if args.local_cache_fallback_dir:
            fallback_min_free_gb = (
                args.local_cache_min_free_gb
                if args.local_cache_fallback_min_free_gb is None
                else args.local_cache_fallback_min_free_gb
            )
            fallback_max_gb = (
                args.local_cache_max_gb
                if args.local_cache_fallback_max_gb is None
                else args.local_cache_fallback_max_gb
            )
            fallback_cache = StaticAnchoredCache(
                args.local_cache_fallback_dir,
                min_free_gb=fallback_min_free_gb,
                max_cache_gb=fallback_max_gb,
            )
            cache_obj = TieredStaticAnchoredCache([primary_cache, fallback_cache])
        else:
            cache_obj = primary_cache

    train_transform = build_train_augmentation(args)
    ...
    core_model = MultiSensorPanopticonClassifier(
        ...
        adapter_first_blocks=args.adapter_first_blocks,
    ...
    if args.data_parallel and device.type == "cuda":
        ...
        dp_gpus = available_gpus
        if args.data_parallel_num_gpus > 0:
            dp_gpus = min(args.data_parallel_num_gpus, available_gpus)
        if dp_gpus > 1:
            dp_device_ids = list(range(dp_gpus))
            print(f"Enabling DataParallel on CUDA device ids {dp_device_ids}", flush=True)
            model = nn.DataParallel(core_model, device_ids=dp_device_ids)
            use_data_parallel = True
        else:
            print("DataParallel requested but insufficient CUDA devices; running single-device.", flush=True)
    elif args.data_parallel:
        print("DataParallel requested on non-CUDA device; running single-device.", flush=True)
    criterion = nn.CrossEntropyLoss(label_smoothing=0.05)
    adapter_blocks = list(getattr(core_model.backbone, "_sensor_adapter_blocks", []))
    trainable_adapter_blocks = select_trainable_adapter_blocks(adapter_blocks, args.adapter_train_blocks)
    adapter_params = list(_gather_adapter_parameters_from_blocks(trainable_adapter_blocks))
    all_adapter_params = list(_gather_adapter_parameters_from_blocks(adapter_blocks))
    trainable_adapter_param_ids = {id(p) for p in adapter_params}
    frozen_adapter_params = [p for p in all_adapter_params if id(p) not in trainable_adapter_param_ids]
    trainable_adapter_block_ids = {id(block) for block in trainable_adapter_blocks}
    param_groups = build_optimizer_param_groups(
        core_model,
        args,
        sensor_head_lr_mult=sensor_head_lr_mult,
        trainable_adapter_blocks=trainable_adapter_blocks,
    )
    ...
    best_score_path = ckpt_dir / "ckpt_best_score.pth"
    ...
    default_threshold = float(args.default_sensor_threshold)
    sensor_thresholds = {sensor: default_threshold for sensor in sensors_list}
    best_sensor_acc = {sensor: float("-inf") for sensor in sensors_list}
    ...
    if isinstance(resume_state, Mapping):
        ...
        if isinstance(saved_thresholds, Mapping):
            for sensor in sensors_list:
                value = saved_thresholds.get(sensor)
                if value is not None:
                    sensor_thresholds[sensor] = float(value)
        saved_best_sensor_acc = resume_state.get("best_sensor_acc")
        for sensor in sensors_list:
            value = saved_best_sensor_acc.get(sensor) if isinstance(saved_best_sensor_acc, Mapping) else None
            if value is not None:
                best_sensor_acc[sensor] = float(value)
    ...
    print(
        ...
        f"adapter_lr={adapter_lr_effective:.2e}, lora_rank={args.adapter_bottleneck_dim}, "
        f"lora_alpha={args.adapter_alpha:.2f}, head_lr={args.head_lr:.2e}, "
        f"sensor_loss_weighting={args.sensor_loss_weighting}, sensor_loss_warmup_epochs={args.sensor_loss_warmup_epochs}, "
        f"train_augment={args.train_augment}, auto_sensor_thresholds={args.auto_sensor_thresholds}, "
        f"early_stopping={args.early_stopping}, early_metric={args.early_stopping_metric}, "
        f"adapter_blocks=train:{len(trainable_adapter_blocks)}/all:{len(adapter_blocks)}, "
        f"freeze_vit_in_adapter_blocks={args.freeze_vit_in_adapter_blocks}",
        flush=True,
    )
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        for p in frozen_adapter_params:
            p.requires_grad = False
        for p in adapter_params:
            p.requires_grad = train_adapters
        for block in adapter_blocks:
            if isinstance(block, SensorAdapterBlock):
                block.adapters.train(train_adapters and (id(block) in trainable_adapter_block_ids))
        ...
        for x_dict, labels, sensors in train_loader:
            ...
            scaler.update()
            optimizer_ran = scaler.get_scale() >= prev_scale
            ...
            if logits is None:
                logits = torch.zeros((batch, core_model.head.out_features), device=device)
        ...
        binary_ready = (
            len(eval_labels) > 0
            and len(eval_labels) == len(eval_preds)
            and len(eval_labels) == len(eval_pos_scores)
            and len(eval_labels) == len(eval_sensors)
        )
        if binary_ready:
            eval_labels_np = np.asarray(eval_labels, dtype=np.int64)
            eval_preds_np = np.asarray(eval_preds, dtype=np.int64)
            eval_scores_np = np.asarray(eval_pos_scores, dtype=np.float64)
            eval_sensors_np = np.asarray(eval_sensors, dtype=object)

            metrics = compute_binary_metrics(
                labels=eval_labels_np,
                preds=eval_preds_np,
                pos_scores=eval_scores_np,
            )
            ...
            if args.auto_sensor_thresholds:
                sensor_threshold_targets, threshold_fit_acc = calibrate_sensor_thresholds(
                    labels=eval_labels_np,
                    scores=eval_scores_np,
                    sensors=eval_sensors_np,
                    sensor_order=sensors_list,
                    min_samples=args.sensor_threshold_min_samples,
                    default_threshold=default_threshold,
                )
                ema = max(0.0, min(1.0, float(args.sensor_threshold_ema)))
                for sensor in sensors_list:
                    target = float(sensor_threshold_targets.get(sensor, default_threshold))
                    previous = float(sensor_thresholds.get(sensor, default_threshold))
                    sensor_thresholds[sensor] = previous * ema + target * (1.0 - ema)

            calibrated_preds = apply_sensor_thresholds(
                scores=eval_scores_np,
                sensors=eval_sensors_np,
                thresholds=sensor_thresholds,
                default_threshold=default_threshold,
            )
            test_acc_calibrated = float((calibrated_preds == eval_labels_np).sum() / max(1, eval_labels_np.size))
            per_sensor_acc_calibrated = compute_per_sensor_accuracy(
                labels=eval_labels_np,
        ...
        selection_score, score_details = compute_early_stopping_score(
            ...
            test_loss=test_loss,
            test_acc=test_acc,
            test_acc_calibrated=test_acc_calibrated,
            per_sensor_acc=per_sensor_acc,
            per_sensor_acc_calibrated=per_sensor_acc_calibrated,
            sensor_order=sensors_list,
            macro_weight=args.early_stopping_macro_weight,
            min_sensor_weight=args.early_stopping_min_sensor_weight,
        )
        min_delta = max(0.0, float(args.early_stopping_min_delta))
        selection_improved = selection_score > (best_selection_score + min_delta)
        ...
        if args.save_per_sensor_best:
            for sensor in sensors_list:
                cand = float(per_sensor_acc_calibrated.get(sensor, float("nan")))
                if not math.isfinite(cand):
                    continue
                if cand > (best_sensor_acc[sensor] + 1e-12):
                    best_sensor_acc[sensor] = cand
                    sensor_best_path = ckpt_dir / f"ckpt_best_sensor_{sensor}.pth"
                    save_checkpoint(
                        sensor_best_path,
                        epoch,
                        global_step,
        ...
        if wandb_run is not None:
            log_payload = {
                ...
                "test_fpr_calibrated": test_fpr_calibrated,
                "test_recall_calibrated": test_recall_calibrated,
                "test_auroc_calibrated": test_auroc_calibrated,
            ...
            if math.isfinite(score_details.get("macro_cal", float("nan"))):
                log_payload["test_acc_macro_calibrated"] = score_details["macro_cal"]
            if math.isfinite(score_details.get("min_raw", float("nan"))):
                log_payload["test_acc_min_sensor"] = score_details["min_raw"]
            if math.isfinite(score_details.get("min_cal", float("nan"))):
                log_payload["test_acc_min_sensor_calibrated"] = score_details["min_cal"]
            for sensor in sensors_list:
