...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="Number of transformer blocks to attach sensor-specific LoRA adapters to.",
    )
    parser.add_argument(
...
def main(args):
    ...
    core_model = MultiSensorPanopticonClassifier(
        ...
        adapter_first_blocks=args.freeze_backbone_first_blocks,
    ...
    phase1_path = ckpt_dir / f"ckpt_phase1_epoch{max(0, int(args.phase1_backbone_epochs))}.pth"
    ...
    if args.resume:
        ...
        if (not latest_path.is_file()) and phase1_path.is_file():
            resume_path = phase1_path
            print(f"[Resume] latest checkpoint not found, fallback to phase1 checkpoint: {phase1_path}", flush=True)
    ...
    freeze_first_blocks = max(0, int(args.freeze_backbone_first_blocks))
    ...
    if isinstance(resume_state, Mapping):
        ...
        # Ensure frozen adapters/heads/patch-embeds are restored to their best checkpoint state.
        for sensor in early_stop_sensors:
    ...
    print(
        ...
        f"freeze_backbone_first_blocks={freeze_first_blocks}, adapter_lr={adapter_lr_effective:.2e}, "
        "phase2_mode=freeze_backbone_train_lora_and_heads, "
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        if in_phase1:
            ...
            # Fixed phase 2 behavior: freeze backbone/patch-embeds, train LoRA adapters + heads.
            set_trainable(core_model.backbone, False)
            core_model.backbone.eval()
            for sensor in core_model.sensor_patch_embeds.values():
                set_trainable(sensor, False)
                sensor.eval()
            for param in adapter_params:
                param.requires_grad = True
            for block in adapter_blocks:
        ...
        if test_acc > best_test_acc:
            ...
            ckpt_extra_state = {
                ...
                "freeze_backbone_first_blocks": freeze_first_blocks,
            }
            save_checkpoint(
                best_path,
                epoch,
        ...
        if phase1_epochs > 0 and epoch == phase1_epochs:
            phase1_extra_state = dict(ckpt_extra_state)
            phase1_extra_state["phase1_completed"] = True
            phase1_extra_state["phase1_checkpoint_epoch"] = epoch
            save_checkpoint(
                phase1_path,
                epoch,
                global_step,
                ...
                best_test_acc,
                args,
                extra_state=phase1_extra_state,
            )
            print(f"[Checkpoint] Saved phase1 checkpoint: {phase1_path}", flush=True)
