...
def _gather_wrapped_block_parameters(blocks: Sequence[nn.Module]) -> Sequence[nn.Parameter]:
    params: list[nn.Parameter] = []
    seen: set[int] = set()
    for block in blocks:
        if not isinstance(block, SensorAdapterBlock):
...
def build_optimizer_param_groups(
    ...
    *,
    ...
    frozen_backbone_param_ids: Optional[set[int]] = None,
) -> list[dict]:
    ...
    if args.train_backbone:
        ...
        if not args.train_adapters_only:
            ...
            if frozen_backbone_param_ids:
                backbone_core = [p for p in backbone_core if id(p) not in frozen_backbone_param_ids]
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Train only the last N adapterized blocks (default: train all adapterized blocks).",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        dest="freeze_vit_in_adapter_blocks",
        help="Freeze base ViT weights inside the last adapter_last_blocks and train only LoRA there.",
    )
    parser.add_argument(
        "--disable_freeze_vit_in_adapter_blocks",
        action="store_false",
        dest="freeze_vit_in_adapter_blocks",
        help="Allow full ViT finetuning inside blocks that also contain LoRA adapters.",
    )
    parser.set_defaults(freeze_vit_in_adapter_blocks=True)
...
def main(args):
    ...
    wrapped_block_params = list(_gather_wrapped_block_parameters(adapter_blocks))
    ...
    frozen_wrapped_block_param_ids = {id(p) for p in wrapped_block_params} if args.freeze_vit_in_adapter_blocks else set()
    param_groups = build_optimizer_param_groups(
        ...
        frozen_backbone_param_ids=frozen_wrapped_block_param_ids,
    )
