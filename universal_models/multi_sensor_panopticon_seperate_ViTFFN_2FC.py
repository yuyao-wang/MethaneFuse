...
class DomainSpecificLinear(nn.Module):
    """Per-domain Linear layer (used for fc1 and now fc2).
...
def _convert_backbone_ffn_fc12(backbone: DinoVisionTransformer, num_domains: int):
    """Make MLP fc1 *and* fc2 domain-specific."""
    if num_domains <= 1:
        return backbone
    if hasattr(backbone, "_domain_specific_fc_linears"):
        return backbone
    ...
    def _maybe_replace(mlp: nn.Module):
        for name in ("fc1", "fc2"):
            fc = getattr(mlp, name, None)
            if isinstance(fc, DomainSpecificLinear) or not isinstance(fc, nn.Linear):
                continue
            ds = DomainSpecificLinear(fc.in_features, fc.out_features, num_domains, bias=fc.bias is not None)
            with torch.no_grad():
                ds.weight.copy_(fc.weight.unsqueeze(0).expand(num_domains, -1, -1))
                if fc.bias is not None:
                    ds.bias.copy_(fc.bias.unsqueeze(0).expand(num_domains, -1))
            setattr(mlp, name, ds)
    ...
    backbone._domain_specific_fc_linears = ds_linears  # type: ignore[attr-defined]
    return backbone
...
class MultiSensorPanopticonClassifier(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # patch ffns to make both fc1 and fc2 sensor-specific (domain-specific)
        _convert_backbone_ffn_fc12(backbone, num_domains=len(self.sensor_order))
