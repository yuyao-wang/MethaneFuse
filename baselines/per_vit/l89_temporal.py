...
def sanitize_input_imgs(x_dict, nan_fill_value: float = 0.0, clip_abs: float = 0.0):
    """Sanitize NaN/Inf values in x_dict['imgs'] to avoid non-finite training states."""

    imgs = x_dict.get("imgs")
    if imgs is None or not isinstance(imgs, torch.Tensor):
        return x_dict

    imgs = torch.nan_to_num(imgs, nan=nan_fill_value, posinf=nan_fill_value, neginf=nan_fill_value)
    if clip_abs is not None and clip_abs > 0:
        imgs = imgs.clamp(min=-clip_abs, max=clip_abs)
    x_dict["imgs"] = imgs
    return x_dict
...
def resolve_l89_path_columns(csv_path: str, requested_cols: Tuple[str, str, str]) -> Tuple[str, str, str]:
    """
    Resolve temporal columns for L89-only training.

    Priority:
      1) user-requested columns (e.g., path_t0/path_t90/path_t360)
      2) wide-table L89 columns (l89_0_path/l89_90_path/l89_360_path)
    """
    try:
    ...
    wide_cols = ("l89_0_path", "l89_90_path", "l89_360_path")
    if all(col in header_set for col in wide_cols):
        print(
            f"[Data] {csv_path}: requested columns {requested_cols} not found; using wide-table L89 columns {wide_cols}.",
            flush=True,
        )
        return wide_cols

    raise ValueError(
        f"Could not resolve temporal L89 path columns for {csv_path}. "
...
def build_l89_prefiltered_csv(
    ...
    *,
    ...
    """
    Build a temporary CSV that contains only L89 rows with valid path cells.

    This avoids pandas NaN(float) path values crashing skip-invalid prechecks in
    the base S2 CSV dataset implementation.
    """

    tmp_dir = os.environ.get("TMPDIR") or None
    fd, tmp_path = tempfile.mkstemp(prefix=f"l89_prefilter_{split_name}_", suffix=".csv", dir=tmp_dir, text=True)
    os.close(fd)
    ...
    with open(csv_path, "r", newline="") as fin, open(tmp_path, "w", newline="") as fout:
        ...
        missing = [c for c in path_columns if c not in reader.fieldnames]
        if missing:
        ...
        for row in reader:
            total += 1
            if has_sensor:
                sensor_val = str(row.get(sensor_column, "")).strip().lower()
                if sensor_val != sensor_name.lower():
                    continue
            if not all(_valid_path_cell(row.get(col)) for col in path_columns):
                continue
            writer.writerow(row)
            kept += 1

    print(
        f"[Data] {split_name}: prefiltered L89 rows {kept}/{total} into temp CSV {tmp_path}.",
        flush=True,
    )
    if kept == 0:
        raise ValueError(f"{split_name}: no valid L89 rows after prefiltering {csv_path}")
    return tmp_path


def filter_dataset_to_l89_only(base_ds, *, split_name: str, sensor_column: str = "sensor", sensor_name: str = "l89") -> None:
    """Filter a temporal dataset in-place to keep only valid L89 rows."""
    ...
    for col in base_ds.path_columns:
        if col not in df.columns:
            raise ValueError(f"{split_name}: missing required path column '{col}' after L89 filtering.")
        df = df[_valid_path_series(df[col])]

    base_ds.df = df.reset_index(drop=True)
    after = len(base_ds.df)
    print(
        f"[Data] {split_name}: kept {after}/{before} rows for L89 using path columns {base_ds.path_columns}.",
        flush=True,
    )
    if after == 0:
        raise ValueError(f"{split_name}: no valid L89 rows available after filtering.")
...
def main(args):
    ...
    train_path_columns = resolve_l89_path_columns(args.train_csv, requested_cols)
    test_path_columns = resolve_l89_path_columns(args.test_csv, requested_cols)
    ...
    temp_csv_paths = []
    train_csv_for_ds = build_l89_prefiltered_csv(
        args.train_csv, train_path_columns, split_name="train", sensor_column=args.sensor_column
    )
    test_csv_for_ds = build_l89_prefiltered_csv(
        args.test_csv, test_path_columns, split_name="test", sensor_column=args.sensor_column
    )
    ...
    filter_dataset_to_l89_only(base_train_ds, split_name="train", sensor_column=args.sensor_column)
    filter_dataset_to_l89_only(base_test_ds, split_name="test", sensor_column=args.sensor_column)
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        correct = 0
        total = 0
        skipped_train_nonfinite = 0
        for step, (x_dict, labels) in enumerate(train_loader, 1):
            labels = labels.to(device)
            x_dict = recursive_to_device(x_dict, device)
            x_dict = sanitize_input_imgs(x_dict, nan_fill_value=args.nan_fill_value, clip_abs=args.input_clip_abs)
            ...
            if not torch.isfinite(logits).all():
                skipped_train_nonfinite += 1
                if skipped_train_nonfinite <= 5:
                    print(f"[Warn] Epoch {epoch} step {step}: non-finite logits; skipping batch.", flush=True)
                continue
            if not torch.isfinite(loss):
                skipped_train_nonfinite += 1
                if skipped_train_nonfinite <= 5:
                    print(f"[Warn] Epoch {epoch} step {step}: non-finite loss; skipping batch.", flush=True)
                continue
            ...
            if args.log_interval and step % args.log_interval == 0:
                running_loss = total_loss / total if total > 0 else float("nan")
        ...
        with torch.no_grad():
            for step, (x_dict, labels) in enumerate(test_loader, 1):
                ...
                if not torch.isfinite(logits).all():
                    skipped_eval_nonfinite += 1
                    if skipped_eval_nonfinite <= 5:
                        print(f"[Warn] Eval epoch {epoch} step {step}: non-finite logits; skipping batch.", flush=True)
                    continue
                if not torch.isfinite(loss):
                    skipped_eval_nonfinite += 1
                    if skipped_eval_nonfinite <= 5:
                        print(f"[Warn] Eval epoch {epoch} step {step}: non-finite loss; skipping batch.", flush=True)
                    continue

                test_loss_total += float(loss.item()) * labels.size(0)
                preds = logits.argmax(dim=1)
                correct += (preds == labels).sum().item()
                total += labels.size(0)
        ...
        print(
            ...
            f"skipped_train_nonfinite={skipped_train_nonfinite} skipped_eval_nonfinite={skipped_eval_nonfinite}",
            flush=True,
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--disable_amp",
        action="store_true",
        help="Disable mixed precision even on CUDA (useful if training becomes unstable).",
    )
    parser.add_argument(
        "--nan_fill_value",
        type=float,
        default=0.0,
        help="Replacement value for NaN/Inf pixels before forwarding to the backbone.",
    )
    parser.add_argument(
        "--input_clip_abs",
        type=float,
        default=50.0,
        help="Clip input tensors to [-input_clip_abs, input_clip_abs]; set <=0 to disable clipping.",
    )
    ...
    parser.add_argument(
        "--sensor_column",
        default="sensor",
        help="CSV column that stores sensor name; used to keep only L89 rows in wide tables.",
    )
    parser.add_argument(
