...
def build_resnet34(num_channels: int = 36, num_classes: int = 2) -> nn.Module:
...
def main(args):
    ...
    print(
        f"ResNet-34 baseline with 3x12 channels | train_samples={len(train_ds)} test_samples={len(test_ds)} "
        f"pad_to_multiple={args.pad_to_multiple}",
        flush=True,
    )

    model = build_resnet34(num_channels=36, num_classes=2).to(device)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Standalone ResNet-34 baseline for S2 temporal classification (3x12 bands).")
