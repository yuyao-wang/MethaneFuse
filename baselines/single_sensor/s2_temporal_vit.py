...
class S2TemporalStackedDataset:
    ...
    - Stacks them into a single 36-channel tensor for a plain ViT baseline.
...
def build_vit(num_channels: int = 36, num_classes: int = 2, image_size: int = 224) -> nn.Module:
    model = models.vit_b_16(weights=None, image_size=image_size)
    # Adapt patch projection to 36 channels.
...
def main(args):
    ...
    print(
        f"ViT-B/16 baseline with 3x12 channels | train_samples={len(train_ds)} test_samples={len(test_ds)} "
    ...
    model = build_vit(num_channels=36, num_classes=2, image_size=args.image_size).to(device)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Standalone ViT-B/16 baseline for S2 temporal classification (3x12 bands).")
    ...
    parser.add_argument("--image_size", type=int, default=224, help="Images are resized to this square size before ViT forward.")
