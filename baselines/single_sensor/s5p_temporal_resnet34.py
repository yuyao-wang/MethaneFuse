...
def build_resnet34(num_channels: int, num_classes: int = 2) -> nn.Module:
...
def main(args):
    ...
    print(
        f"S5P ResNet-34 baseline with {num_channels} input channels | "
    ...
    model = build_resnet34(num_channels=num_channels, num_classes=2).to(device)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Standalone ResNet-34 baseline for Sentinel-5P temporal classification (stacked 3 timepoints)."
    )
    ...
    parser.add_argument("--resize_size", type=int, default=224, help="Spatial resize before feeding ResNet-34.")
